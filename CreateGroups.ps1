...
Function CreateGroups {
    ...
    foreach ($SGHandle in $file_Groups)
    {
        ...
        If($VrfUser -eq $SGHandle) {
            ...
            If($CharArray[3] -eq $DW) {
                # El grupo no existe y es DATAWRITER
                $desc = "DB UAT DATAWRITER Acceso a base de datos $name"
                New-ADGroup -Name $SGHandle -SamAccountName $SGHandle -GroupCategory Security -GroupScope Global -DisplayName $SGHandle -Path $SGOU -Description $desc
                write-host "Se Creo el Grupo --> $SGHandle" -ForegroundColor green
                write-host ""

            }elseif ($CharArray[3] -eq $DR) {
                # El grupo no existe y es DATAWRITER
                $desc = "DB UAT DATAREADER Acceso a base de datos $name"
                New-ADGroup -Name $SGHandle -SamAccountName $SGHandle -GroupCategory Security -GroupScope Global -DisplayName $SGHandle -Path $SGOU -Description $desc
                write-host "Se Creo el Grupo --> $SGHandle" -ForegroundColor green
                write-host ""

            }elseif ($CharArray[3] -eq $OW) {
                # El grupo no existe y es DATAWRITER
                $desc = "DB UAT OWNER Acceso a base de datos $name"
                New-ADGroup -Name $SGHandle -SamAccountName $SGHandle -GroupCategory Security -GroupScope Global -DisplayName $SGHandle -Path $SGOU -Description $desc
                write-host "Se Creo el Grupo --> $SGHandle" -ForegroundColor green
                write-host ""
